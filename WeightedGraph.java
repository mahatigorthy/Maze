...
public class WeightedGraph<V> {
	...
	private static class Edge<V> {
		...
		public Edge(V neighbor, int weight){
	...
	/** Initializes the Adjacency List to "empty", including
	...
	public boolean containsVertex(V vertex) {
		for(V graphVertex : graph.keySet()) {
			if(graphVertex.equals(vertex)) {
				return true;
			}
		}
		return false;
	...
	public void addEdge(V from, V to, Integer weight) {
		if(graph.containsKey(from) && graph.containsKey(to) && weight >= 0) {
			graph.get(from).add(new Edge<V>(to, weight));
		} else {
			throw new IllegalArgumentException();
		}
	}
	...
	public Integer getWeight(V from, V to) {
		if(graph.containsKey(from) && graph.containsKey(to)) {
			for(Edge<V> edge : graph.get(from)) { // Traverse the neighbors of a vertex
	...
	public void DoBFS(V start, V end) {
		for(GraphAlgorithmObserver<V> observer : observerList) {
			observer.notifyBFSHasBegun(); // Call on each observer
		}
		
		Set<V> visitedSet = new HashSet<>(); // VisitedSet
		Queue<V> queue = new LinkedList<V>(); // Queue initialized as a LinkedList
		queue.add(start); // Add the first vertex to the queue
		
		while(!queue.isEmpty()) {
			V removedVertex = queue.remove(); // Take X out of the Queue
			
			if(!visitedSet.contains(removedVertex)) { // If X is not in the VisitedSet
				visitedSet.add(removedVertex); // Visit X
				
				if(!removedVertex.equals(end)) { // Check if we've reached the end
					for(GraphAlgorithmObserver<V> observer : observerList) {
						observer.notifyVisit(removedVertex);
					}
					
					// Traverse through adjacencies of X
					for(Edge<V> e : graph.get(removedVertex)) {
						if(!visitedSet.contains(e.neighbor)) {
							queue.add(e.neighbor); // add adjacency to the Queue if not in the VisitedSet
						}
					}
				} else { //We've reached the endVertex that we've been searching for
					for(GraphAlgorithmObserver<V> observer : observerList) {
	...
	public void DoDFS(V start, V end) {
		for(GraphAlgorithmObserver observer : observerList) {
			observer.notifyDFSHasBegun(); // Call on each observer
		}
		
		Set<V> visitedSet = new HashSet<>(); // VisitedSet
		Stack<V> discovered = new Stack<V>();
		discovered.push(start); // Push the starting vertex onto the Stack
				
		while(!discovered.isEmpty()) {
			V removedVertex = discovered.pop(); // Pop X from the stack
			
			if(!visitedSet.contains(removedVertex)) { // Checks if we are at the end of the search
				for(GraphAlgorithmObserver<V> observer : observerList) {
					observer.notifyVisit(removedVertex);
				}
				
				if(!removedVertex.equals(end)) { // If X is not in the VisitedSet
					visitedSet.add(removedVertex); //Visit X
					
					// Traverse through adjacencies of X
					for(Edge<V> edge : graph.get(removedVertex)) {
						if(!visitedSet.contains(edge.neighbor)) { //If adjacency is not in the visited set
							discovered.push(edge.neighbor); //Push adjacency onto the Stack
						}
					}
	...
	public void DoDijsktra(V start, V end) {
		for(GraphAlgorithmObserver<V> observer : observerList) {
			observer.notifyDijkstraHasBegun(); //Call on each observer
		}
		
		Set<V> finishedSet = new HashSet<>(); //Finished Set
		Map<V, V> predecessors = new HashMap<>(); // Maps the vertices to their predecessors
		Map<V, Integer> cost = new HashMap<>(); // Maps the vertices to their costs

		for(V vertex : graph.keySet()) {
			predecessors.put(vertex, null); //Null for all vertices in the beginning
		}
		...
		while(!finishedSet.containsAll(graph.keySet())) { //While not all vertices are in the FinishedSet
			
			// Find vertex K that is not in the FinishedSet and has the smallest cost
			...
			// Add K to the FinishedSet
			finishedSet.add(currentVertex);
			
			for(GraphAlgorithmObserver<V> observer : observerList) {
				observer.notifyDijkstraVertexFinished(currentVertex, cost.get(currentVertex)); 
			}
			
			for(Edge<V> edge : graph.get(currentVertex)) {
				if(!finishedSet.contains(edge.neighbor)) { // For each neighbor of K that is not in FinishedSet
					if(cost.get(currentVertex) + edge.weight < cost.get(edge.neighbor)) {
						//Adjust the costs and predecessors if they are less than the current ones
						cost.put(edge.neighbor, cost.get(currentVertex) + edge.weight);
						predecessors.put(edge.neighbor, currentVertex);
					}
				}
			}
		...
		V currentVertex = end;
		LinkedList<V> leastCostPath = new LinkedList<V>(); 
		leastCostPath.add(end);
		//Traverse between the predecessors
		while(!currentVertex.equals(start)) {
			currentVertex = predecessors.get(currentVertex);
			leastCostPath.addFirst(currentVertex);
		}
